const messages = {
  en: {
    ...
    foreignSizeGeneric: "sized by another client ({cols}x{rows}) — type to take over",
    foreignSizeNamed: "sized by {name} ({cols}x{rows})",
  ...
  ja: {
    ...
    foreignSizeGeneric: "別のクライアントがサイズを設定中（{cols}x{rows}）— 入力すると引き継ぎます",
    foreignSizeNamed: "{name} がサイズを設定中（{cols}x{rows}）",
    thisDevice: "このデバイス",
    disconnect: "切断",
    unnamed: "名前なし",
  },
